...
cap_left = cv2.VideoCapture(pipeline_str, cv2.CAP_GSTREAMER)
cap_right = cv2.VideoCapture(pipeline_str2, cv2.CAP_GSTREAMER)
...
with mp_facedetector.FaceDetection(min_detection_confidence=0.7) as face_detection:
    ...
    print("222222")
    while(cap_right.isOpened() and cap_left.isOpened()) or True:
        ...
        if not succes_right or not succes_left:                    
            ...
            if results_right.detections:
                for id, detection in enumerate(results_right.detections):
                    ...
                    cv2.putText(frame_right, '{}%'.format(int(detection.score[0]*100)), (boundBox[0], boundBox[1] - 20), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,255,0), 2)
            ...
            if results_left.detections:
                for id, detection in enumerate(results_left.detections):
                    ...
                    cv2.putText(frame_left, '{}%'.format(int(detection.score[0]*100)), (boundBox[0], boundBox[1] - 20), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,255,0), 2)
            ...
            cv2.putText(frame_right,  'FPS: {}'.format(int(fps)), (20,450), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0,255,0), 2)
            cv2.putText(frame_left,  'FPS: {}'.format(int(fps)), (20,450), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0,255,0), 2)                                   
